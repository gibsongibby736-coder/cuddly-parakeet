...
// ══════════════════════════════════════════════
//  SHADOW AI — WhatsApp Bot v12 (Admin Fix)
...
// ══════════════════════════════════════════════
//  التحقق من صلاحيات البوت (إصدار محسن)
// ══════════════════════════════════════════════
async function isBotAdmin(sock, groupJid) {
  try {
    const groupMetadata = await sock.groupMetadata(groupJid);
    
    // الحصول على رقم البوت (بدون الامتداد)
    const botNumber = sock.user.id.split(':')[0];
    const botJid = `${botNumber}@s.whatsapp.net`;
    
    console.log(`🔍 معرف البوت: ${botJid}`);
    
    // البحث عن البوت في قائمة المشرفين
    const isAdmin = groupMetadata.participants.some(participant => {
      const participantNumber = participant.id.split('@')[0];
      return participantNumber === botNumber && (participant.admin === 'admin' || participant.admin === 'superadmin');
    });
    
    console.log(`👑 هل البوت مشرف في ${groupJid}؟ ${isAdmin ? 'نعم ✅' : 'لا ❌'}`);
...
async function startBot() {
  ...
  console.log('👑 البوت سيرد فقط في المجموعات التي هو مشرف فيها');
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === 'open') {
      ...
      console.log(`🔢 رقم البوت: ${sock.user.id}`);
      await tgSendText(`✅ *Shadow AI متصل بواتساب!*\n\n🌑 البوت يعمل الآن.\n🔢 رقم البوت: \`${sock.user.id}\`\n👑 يرد فقط في المجموعات التي هو مشرف فيها.`);
    }
  });
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        // ═══════════════════════════════════════════
        // ✅ التحقق: البوت يرد فقط إذا كان مشرفاً
        // ═══════════════════════════════════════════
