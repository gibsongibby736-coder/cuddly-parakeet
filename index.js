...
const SYSTEM_PROMPT =
  ...
  '4. If the user sends an image with questions or exercises, READ the image carefully and answer ALL the questions you see. ' +
  ...
  if (imgB64) {
    messages.push({
      ...
      content: [
        ...
        {
          type: 'text',
          text: `${userName}: ${text || 'اقرأ هذه الصورة بعناية وأجب على كل ما فيها من أسئلة أو تمارين.'}`
        }
      ]
    });
  } else {
  ...
  try {
    const res = await axios.post(
      'https://text.pollinations.ai/openai',
      {
        ...
        messages,
        temperature: 0.7,
        max_tokens: 2000, // زيادة الحد للردود الطويلة
        private: true,
      },
    ...
    for (const msg of messages) {
      try {
        ...
        if (mc.conversation) {
          ...
          try {
            // الطريقة الصحيحة لتحميل الصور في Baileys الجديد
            ...
            // إخبار المستخدم أن الصورة لم تُحمَّل
