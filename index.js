...
// ══════════════════════════════════════════════
//  SHADOW AI — WhatsApp Bot v16 (Gemini 2.5 Flash)
//  بدون systemInstruction + سرعة أعلى
// ══════════════════════════════════════════════
...
// ══════════════════════════════════════════════
//  PROMPT (بدون systemInstruction)
// ══════════════════════════════════════════════
const USER_PROMPT = `أنت مساعد ذكاء اصطناعي متقدم داخل مجموعة واتساب.

القواعد:
1. رد بنفس لغة المستخدم (عربية، إنجليزية، فرنسية، دارجة).
2. إذا سأل المستخدم سؤالاً قصيراً، أعط رداً قصيراً.
...
// ══════════════════════════════════════════════
//  تهيئة Gemini 2.5 Flash (بدون systemInstruction)
// ══════════════════════════════════════════════
const genAI = new GoogleGenerativeAI(GEMINI_API_KEY);

const model = genAI.getGenerativeModel({ 
  model: "gemini-2.5-flash",  // ✅ أحدث نموذج
  generationConfig: {
    temperature: 0.7,
    topP: 0.95,
    topK: 40,
    maxOutputTokens: 4096,    // ✅ زيادة الحد
  }
}, { apiVersion: 'v1' });

// ══════════════════════════════════════════════
//  نظام الطوابير (محسن للضغط العالي)
// ══════════════════════════════════════════════
const userQueues = new Map();
const userProcessing = new Map();
const MAX_QUEUE_SIZE = 20;  // ✅ زيادة من 10 إلى 20
  ...
  while (userQueues.get(userId)?.length > 0) {
    ...
    if (task) {
      try {
        await Promise.race([
          task(),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Task timeout')), 60000))  // ✅ زيادة المهلة
        ]);
      } catch (e) {
        console.error(`❌ خطأ:`, e.message);
      }
      await new Promise(r => setTimeout(r, 1000));  // ✅ تقليل التأخير
    }
  }
...
// ══════════════════════════════════════════════
//  ذاكرة المحادثات (زيادة إلى 15 رسالة)
// ══════════════════════════════════════════════
const histories = new Map();
const MAX_HISTORY = 15;  // ✅ زيادة من 10 إلى 15
...
async function sendQR(qrData) {
  ...
  try {
    ...
    form.append('caption', '🤖 *البوت الجديد — امسح للربط*\n\n⏱ ينتهي خلال 60 ثانية');
