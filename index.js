...
// ══════════════════════════════════════════════
//  SHADOW AI — WhatsApp Bot v11 (Admin Only + Optimized)
//  يرد فقط في المجموعات التي هو مشرف فيها
//  مع تحسينات: superadmin, systemInstruction, مهلة أقل
// ══════════════════════════════════════════════
...
if (!GEMINI_API_KEY) {
  ...
  console.log('📌 أضف المتغير في Railway → Variables');
  process.exit(1);
}

// ══════════════════════════════════════════════
//  SYSTEM PROMPT (سيتم تمريره كـ systemInstruction)
// ══════════════════════════════════════════════
...
// ══════════════════════════════════════════════
//  تهيئة Gemini (مع systemInstruction)
// ══════════════════════════════════════════════
const genAI = new GoogleGenerativeAI(GEMINI_API_KEY);

const model = genAI.getGenerativeModel({ 
  model: "gemini-1.5-flash",
  systemInstruction: SYSTEM_PROMPT, // ✅ الطريقة الصحيحة (توفر توكنز)
  generationConfig: {
    temperature: 0.7,
    topP: 0.95,
    topK: 40,
  ...
  while (userQueues.get(userId)?.length > 0) {
    ...
    if (task) {
      try {
        await Promise.race([
          task(),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Task timeout')), 45000)) // ✅ 45 ثانية فقط
        ]);
      } catch (e) {
...
// ══════════════════════════════════════════════
//  ذاكرة المحادثات (10 رسائل فقط لكل مستخدم)
// ══════════════════════════════════════════════
const histories = new Map();
const MAX_HISTORY = 10;

// ══════════════════════════════════════════════
//  التحقق من صلاحيات البوت (يدعم admin و superadmin)
// ══════════════════════════════════════════════
...
// ══════════════════════════════════════════════
//  AI مع سياق محدود (بدون إعادة إرسال system prompt)
// ══════════════════════════════════════════════
async function askAI(userId, groupId, text, userName, imgB64, imgMime, audioText) {
  ...
  // ✅ بدون system prompt هنا (لأنه عرفناه في النموذج)
  ...
  const parts = [];
  // ✅ نص المستخدم فقط (بدون إعادة التعليمات)
  ...
  try {
    ...
    if (e.message.includes('429') || e.message.includes('quota') || e.message.includes('rate limit')) {
      return '⚠️ تم تجاوز عدد الطلبات المسموح بها (15 طلب/دقيقة). انتظر قليلاً ثم حاول مرة أخرى.';
    }
...
async function startBot() {
  ...
  console.log('👑 البوت سيرد فقط في المجموعات التي هو مشرف فيها (admin/superadmin)');
  console.log('📊 الحدود: 1500 طلب/يوم، 15 طلب/دقيقة، ذاكرة 10 رسائل');
  console.log('⏱ مهلة الطلب: 45 ثانية');
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        // ═══════════════════════════════════════════
        // ✅ التحقق: البوت يرد فقط إذا كان مشرفاً (admin أو superadmin)
        // ═══════════════════════════════════════════
        const botIsAdmin = await isBotAdmin(sock, jid);
        if (!botIsAdmin) {
          console.log(`⏭️ تجاهل المجموعة (البوت ليس مشرفاً): ${jid}`);
          continue;
        }
